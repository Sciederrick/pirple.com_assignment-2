...
handlers.index = (data, callback)=>{
  ...
  if(data.method == 'get'){
    ...
    let templateData = {
      'head.title' : 'eat pizza',
      'head.description' : "Pizza at your doorstep, make your order here and we'll do the rest. You are just one click away from a crusty delicious pizza.",
      'body.class' : 'index'
    }
...
handlers.accountCreate = (data, callback)=>{
  ...
  if(data.method == 'get'){
    ...
    let templateData = {
      ...
      'head.description' : "Please fill in the form details to create your account.",
...
// Display Menu
handlers.displayMenu = (data, callback)=>{
  //  Reject any request that isn't a GET
  if(data.method == 'get'){
    //  Prepare data for interpolation
    let templateData = {
      'head.title' : 'Our Menu',
      'head.description' : "Our Pizza Pallette.",
      'body.class' : 'menuList'
    }
    //  Read in a template as a string
...
handlers.viewCart = (data, callback)=>{
  ...
  if(data.method == 'get'){
    //  Prepare data for interpolation
    let templateData = {
      'head.title' : 'pizza|cart',
      'head.description' : "Shopping cart.",
...
handlers._users.post = (data, callback)=>{
  ...
  console.log({firstName, lastName, email, streetAddress, password})
          ...
          if(!err){
            ...
            if(count > 0){
              ...
              for(let i = 0; i < count; i++){
                _data.read('menu', data[i], (err, data)=>{
                  if(!err){
                    temp.push('ok'); // dummy array to track iteration count to fix bug :) (async code and for loops)
                    menuList.push(data)
...
handlers._cart.post = (data, callback)=>{
  ...
  const quantity = typeof data.payload.quantity == 'string' && data.payload.quantity.trim().length > 0 && data.payload.quantity.trim() != '0' ? data.payload.quantity.trim() : false
              ...
              if(!err){
                if(data.length > 0){
                  ...
                  data.forEach((product, index)=>{
                    _data.read(`cart\/${email}`, product, (err, data)=>{
                      if(!err){
                        cart.push(data)
                        if(totalItems == cart.length){
                          //  We now have an array of objects with productId and quantity
                          //  Use productId to get detailed product information
                          cart.forEach((cartItem, index)=>{
                            _data.read('menu', cartItem.productId, (err, data)=>{
                              if(!err){
                                myCart.push({"item":data, "quantity":cartItem.quantity})
                                if(totalItems === myCart.length){
                                  callback(200, {'data':myCart})
                                }
                              }else{
                                callback(500, {'Error':'No such file'})
                              }
                            })
                          })
                        }
...
//  @cart - put (add/subtract quantity from cart)
//  Required data: email, id, quantity
//  Optional data: none
    ...
    handlers._tokens.verifyToken(token, email, (tokenIsValid)=>{
      if(tokenIsValid){
        if(quantity){
          _data.list('cart', (err, data)=>{
            if(!err){
              ...
              if(data.indexOf(email) > -1){
                _data.list(`cart\/${email}`, (err, data)=>{
                  if(!err){
                    if(data.indexOf(id) > -1){
                      ...
                      _data.update(`cart\/${email}`, id, data, (err)=>{
                        if(!err){
                          callback(200)
                        }else{
                      ...
                      callback(400, {'Error':`Product with the id of ${id} cannot be found`})
                    }
                  }else{
                    callback(500, {'Error':'Error listing files'})
                  }
                ...
                callback(200, {'Info':'Your cart is empty, nothing to update'})
              }
            }else{
              callback(500, {'Error':'Error listing files'})
            }
          ...
          callback(400, {'Error':'Missing or wrong quantity format'})
        }
      }else{
        callback(403, {'Error':'Missing required token in header, or token is invalid'})
      }
...
//  Container for all checkout methods
handlers._checkout = {}
...
handlers._checkout.post = (data, callback)=>{
  ...
  if(email){
    handlers._tokens.verifyToken(token, email, (tokenIsValid)=>{
      if(tokenIsValid){
        let dummyCount = [] // resolving bug due to async behaviour
        _data.list(`cart/${email}`, (err, data)=>{
          if(!err){
            numberOfItems = data.length
            data.forEach((item, index)=>{
              _data.read(`cart/${email}`, item, (err, data)=>{
                let quantity = data.quantity
                if(!err){
                  _data.read('menu', item, (err, data)=>{
                    if(!err){
                      let lineItemsObject = {}
                      lineItemsObject.price_data = {
                        currency: 'usd',
                        product_data:{
                          name: data.name
                        },
                      ...
                      lineItemsObject.quantity = quantity
                      lineItems.push(lineItemsObject)
                      dummyCount.push('count');
                      if(dummyCount.length == numberOfItems){
                        stripeCheckout()
                      }
                    }else{
                      callback(500, {'Error':'Error reading file'})
            ...
            callback(500, {'Error':'Error listing cart'})
          }
        })
        const stripeCheckout = async()=>{
          try{
            const session = await stripe.checkout.sessions.create({
              success_url : `http://localhost:3000/success?id={CHECKOUT_SESSION_ID}&email="${email}"`,
              cancel_url : 'http://localhost:3000/cancel',
              payment_method_types : ['card'],
              mode : 'payment',
              line_items: lineItems
            })
              callback(200, {id:session.id}) // use the session id in stripe.redirectToCheckout function
          }catch(err){
            callback(500, err)
          }
        }
...
//  @success_url
handlers.success = (data, callback)=>{
  const acceptableMethods = ['get']
  if(acceptableMethods.indexOf(data.method) > -1){
    handlers._success[data.method](data, callback)
  }else{
    callback(405)
  }
}

//  Container for success_url method
handlers._success = {}

//  @success - get
//  Required data: id
//  Optional data: none
handlers._success.get = (data, callback)=>{
  const id = typeof data.queryStringObject.id == 'string' && data.queryStringObject.id.length > 0 ? data.queryStringObject.id.trim() : false
      ...
      try{
        ...
        if(session){
          ...
          let msg = {
            ...
            totalBilled: session['amount_total'],
          ...
          let templateData = {
            ...
            'head.description' : 'transaction successfull, await your pizza delivery in minutes.',
            'body.class' : 'stripeSuccess'
          }
          //  Read in a template as a string
          helpers.getTemplate('stripeSuccess', templateData, (err, str)=>{
            if(!err && str){
              //  Add the universal header and footer
              helpers.addUniversalTemplates(str, templateData, (err, str)=>{
                if(!err && str){
          ...
          //  send success message to customer inbox
          send(email, msg, (err, data)=>{
            if(err){
              callback(408, {'Error':`Email forwarding failed with ${err}, stripe transaction is complete`})
            }
          })
        }
      }catch(err){
        callback(500, {'Error':'Error retrieving session'})
      }
    })()
  }else{
    callback(400, {'Error':'Missing required field'})
...
handlers.cancel = (data, callback)=>{
  const acceptableMethods = ['get']
  if(acceptableMethods.indexOf(data.method) > -1){
    handlers._cancel[data.method](data, callback)
  }else{
    callback(405)
  }
}

//  Container for cancel_url method
handlers._cancel = {}

//  @cancel - get
//  Required data: none
//  Optional data: none
handlers._cancel.get = (data, callback)=>{
  //  load cancel page
  //  Prepare data for interpolation
  let templateData = {
    'head.title' : 'Payment cancelled',
    'head.description' : 'Payment Transaction cancelled.',
    'body.class' : 'stripeCancel'
  }
  //  Read in a template as a string
  helpers.getTemplate('stripeCancel', templateData, (err, str)=>{
    if(!err && str){
      //  Add the universal header and footer
      helpers.addUniversalTemplates(str, templateData, (err, str)=>{
        if(!err && str){
